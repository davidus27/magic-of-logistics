...
| [`milestones/`](milestones/) | Closed milestone reports. History, not instructions — they describe the superseded control-profile framing this build has moved past. |

## Specification map

Section numbers are preserved in every heading, so a code comment that reads
"section 15.7" still resolves. Deleted sections leave gaps in the numbering rather
than shift everything below them. Find the owning file here.

| §    | Topic | File |
|---|---|---|
| 1–7, 44 | Purpose, scope, technical terms, objective, run duration | [`spec/00-overview.md`](spec/00-overview.md) |
| 8 | Game states: start screen, instructions, run, pause, portal cast, success, failure | [`spec/01-game-flow.md`](spec/01-game-flow.md) |
...
| 19 | Cargo control method A — automatic path movement | [`spec/04-cargo.md`](spec/04-cargo.md) |
| 22 | Input priority rules | [`spec/12-input-and-acceptance.md`](spec/12-input-and-acceptance.md) |
...
| 35 | Run seed rules | [`spec/10-architecture.md`](spec/10-architecture.md) |
| 40 | MVP acceptance criteria | [`spec/12-input-and-acceptance.md`](spec/12-input-and-acceptance.md) and the checklist in [`status.md`](status.md) |
